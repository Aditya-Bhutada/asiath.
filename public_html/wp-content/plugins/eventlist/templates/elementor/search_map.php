		...
		<div class="wrap_search">
			...
			<div id="result_search" class="job_listings_search" style="<?php echo esc_attr($args['show_map'] == '' ? 'width: 100%; padding: 0;' : ''); ?>">
				...
				<?php if ($args['show_filter'] == 'yes') { ?>
					...
					<form class="job_filters" autocomplete="nope" autocorrect="off" autocapitalize="none">
								...
								if( strpos($key,'pos') !== false ){
									switch ( $args[$key] ) {
										...
										<div class="label_search wrap_search_keywords">
											<input type="text" name="keywords" autocomplete="nope" autocorrect="off" autocapitalize="none" placeholder="<?php esc_attr_e( 'What are you looking for?', 'eventlist' ); ?>" value="<?php echo esc_attr($selected_name_event); ?>"/>
										</div>
										<?php
										break;
										...
										<div class="label_search wrap_search_location">
											...
											<input type="hidden" name="map_lat" id="map_lat" autocomplete="nope" autocorrect="off" autocapitalize="none"/>
											<input type="hidden" name="map_lng" id="map_lng" autocomplete="nope" autocorrect="off" autocapitalize="none"/>

											<input type="text" id="pac-input" name="map_address" value="" class="controls" placeholder="<?php esc_attr_e( 'Location', 'eventlist' ); ?>" autocomplete="nope" autocorrect="off" autocapitalize="none">
											<i class="locate-me icon_circle-slelected" id="locate-me"></i>
											...
											<input type="hidden" value="" name="map_name" id="map_name"  autocomplete="nope" autocorrect="off" autocapitalize="none"/>
										</div>
										<?php
										break;
										...
										<div class="label_search wrap_search_cat">
											...
											<select name="cat" class="selectpicker">
												...
												<?php foreach ($cats as $cat) { ?>
													<option value="<?php echo esc_attr( $cat->slug ); ?>" <?php echo esc_attr( ($cat->slug == $selected_cat) ? 'selected' : '' ); ?> ><?php echo esc_html( $cat->name ); ?></option>
												<?php } ?>
											</select>
										</div>
										<?php
										...
										<div class="label_search wrap_search_start_date">
											<input type="text" class="el_select_date" placeholder="<?php esc_attr_e('Start date ...', 'eventlist'); ?>" name="start_date" value="<?php echo esc_attr($selected_start_date); ?>" data-format="<?php echo esc_attr( $format ); ?>" autocomplete="nope" autocorrect="off" autocapitalize="none" />
										</div>
										<?php
										break;
										...
										<div class="label_search wrap_search_end_date">
											<input type="text" class="el_select_date" placeholder="<?php esc_attr_e('End date ...', 'eventlist'); ?>" name="end_date" value="<?php echo esc_attr($selected_end_date); ?>" data-format="<?php echo esc_attr( $format ); ?>" autocomplete="nope" autocorrect="off" autocapitalize="none" />
										</div>
										<?php
										break;
						...
						<div class="wrap_search_radius">
							...
							<input type="hidden" value="<?php echo apply_filters( 'el_map_range_radius', 50 ); ?>" name="radius">
						</div>
				...
				<div id="search_result" class="search_result" data-type="<?php echo esc_attr($archive_type); ?>" data-column="<?php echo esc_attr($archive_column); ?>" data-zoom="<?php echo esc_attr($zoom); ?>" data-lat="<?php echo esc_attr($lat_default); ?>" data-lng="<?php echo esc_attr($lng_default); ?>" data-marker_option="<?php echo esc_attr($marker_option); ?>" data-marker_icon="<?php echo esc_attr($marker_icon); ?>">
					<div class="event_archive <?php echo esc_attr($archive_type . ' '. $archive_column); ?> " >
						...
						if($events->have_posts() ) : while ( $events->have_posts() ) : $events->the_post();
							...
							data-average_rating="<?php echo get_average_rating_by_id_event( $id ); ?>"
							data-number_comment="<?php echo get_number_coment_by_id_event( $id ); ?>"
							data-map_lat_event="<?php echo get_post_meta( $id, OVA_METABOX_EVENT.'map_lat', true ); ?>"
							data-map_lng_event="<?php echo get_post_meta( $id, OVA_METABOX_EVENT.'map_lng', true ); ?>"
			...
			<div class="wrap_show_map" style="<?php echo $show_map; ?>" >
				<div id="show_map"></div>
			</div>
		</div>
